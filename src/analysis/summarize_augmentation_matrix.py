"""Compare real-only, copy-paste, clean SDXL, and degraded SDXL evaluations."""
...
SERIES = (
    ("CP-B", "Copy-paste"),
    ("SDXL-C", "SDXL clean"),
    ("SDXL-M", "SDXL mixed degradation"),
)
QUANTITIES = (512, 1024, 1536, 2048)


def result_path(prefix: str, quantity: int) -> Path:
    return EVALUATION_ROOT / f"{prefix}{quantity:04d}_yolo11s_seed0_results.json"
...
def main() -> None:
    baseline_path = EVALUATION_ROOT / "E000_results.json"
    baseline = metrics(baseline_path)
    rows = []
    for prefix, label in SERIES:
        for quantity in QUANTITIES:
            path = result_path(prefix, quantity)
    ...
    csv_path = EVALUATION_ROOT / "augmentation_matrix_summary.csv"
    ...
    colors = {"CP-B": "#777777", "SDXL-C": "#3366cc", "SDXL-M": "#b52b65"}
    ...
    axes[0, 0].legend()
    fig.tight_layout()
    plot_path = EVALUATION_ROOT / "augmentation_matrix_summary.png"
    fig.savefig(plot_path, dpi=180)
    plt.close(fig)
    print(json.dumps({"rows": len(rows), "csv": str(csv_path), "plot": str(plot_path)}, indent=2))
